def int_from_str_finder(input_str, spelt_digits=False):
        ...
        if not spelt_digits:
            ...
            for char in line:
                if char.isnumeric():
                    ...
                    potential_digit += char
                    potential_digit_len = len(potential_digit)
                    ic = None
                    if potential_digit_len >= 5:
                        for k in digit_dict_5:
                            if k == potential_digit[-5:]:
                                ic = digit_dict_5[k]
                                break
                    if potential_digit_len >= 4:
                        for k in digit_dict_4:
                            if k == potential_digit[-4:]:
                                ic = digit_dict_4[k]
                                break
                    if potential_digit_len >= 3:
                        for k in digit_dict_3:
                            if k == potential_digit[-3:]:
                                ic = digit_dict_3[k]
                                break
                    if ic is not None:
                        if first is None:
...
39njjvzt7threetkccstz'''
    print(int_from_str_finder(test))
    print(int_from_str_finder(test, spelt_digits=True))
