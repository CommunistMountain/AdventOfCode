...
def sum_of_possible_games(input_str, find_min_possible_power=False):
    lim_dict = {
        "red": 12,
        "green": 13,
        "blue": 14
    ...
    for line in input_arr:
        game_no, subgames = line.split(':')
        num = game_no.split(' ')[1]
        subgame_arr = [x[1:] for x in subgames.split(';')]
        flag = True
        temp_min_dict = {
            "red": 0,
            "green": 0,
        ...
        for subgame in subgame_arr:
            kv_arr = subgame.split(', ')
            for kv in kv_arr:
                v, k = kv.split(' ')
                iv = int(v)
                if lim_dict[k] < iv:
                    flag = False
                if temp_min_dict[k] < iv:
                    temp_min_dict[k] = iv
        final_power += prod(temp_min_dict.values())
        if flag:
            final += int(num)
    if not find_min_possible_power:
        return final
    else:
        return final_power
...
Game 100: 4 blue, 3 green; 5 blue, 12 green; 16 green, 1 red, 1 blue; 2 blue, 1 green; 1 red, 3 blue, 18 green; 3 green, 1 red, 3 blue'''
    print(sum_of_possible_games(test))
    print(sum_of_possible_games(test,find_min_possible_power=True))
